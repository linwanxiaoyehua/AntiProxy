...
// Sensitive credentials read from environment variables, with default values as fallback (for development only)
...
pub async fn refresh_access_token(refresh_token: &str) -> Result<TokenResponse, String> {
    ...
    if response.status().is_success() {
        ...
        crate::modules::logger::log_info(&format!("Token refresh successful! Valid for: {} seconds", token_data.expires_in));
        Ok(token_data)
    } else {
        let error_text = response.text().await.unwrap_or_default();
...
pub async fn ensure_fresh_token(
    ...
    Ok(crate::models::TokenData::new(
        ...
        current_token.refresh_token.clone(), // Refresh may not return a new refresh_token
        response.expires_in,
